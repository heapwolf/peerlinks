...
const sodiumUniversal = require('sodium-universal')

const { now, BanError, EphemeralBox } = require('../utils')

const {
  Identity: PIdentity,
  Invite: PInvite,
  InviteRequest: PInviteRequest
} = require('../messages')
...
const LINK_EXPIRATION_DELTA = Link.EXPIRATION_DELTA
const LINK_EXPIRATION_LEEWAY = Link.EXPIRATION_LEEWAY

const debug = createDebug('peerlinks:identity')

const MAX_INVITE_NAME_LENGTH = 128
const INVITE_REQUEST_ID_LENGTH = 32
...
class Identity {
  ...
  issueInvite (channel, requestData, trusteeDisplayName) {
    ...
    if (request.peerId.length > ID_LENGTH) {
  ...
  inviteRequestIdFor (publicKey) {
    ...
    sodiumUniversal.crypto_generichash(requestId, publicKey, 'peerlinks-invite')
    return requestId
  }
