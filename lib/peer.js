...
const Chain = require('./protocol/chain')
const Channel = require('./protocol/channel')
...
class Peer {
  ...
  async onQueryResponse (channel, seq, packet) {
    for (const abbr of packet.abbreviatedMessages) {
      Message.checkHash(abbr.hash, `Invalid abbreviated message hash (${abbr.hash})`)
      for (const hash of abbr.parents) {
        Message.checkHash(hash, `Invalid abbreviated message parent hash (${hash})`)
      }
    }
...
Peer.PING_INTERVAL_MIN = PING_INTERVAL_MIN
Peer.PING_INTERVAL_MAX = PING_INTERVAL_MAX
Peer.PONG_TIMEOUT = PONG_TIMEOUT
Peer.MAX_PING_RETRIES = MAX_PING_RETRIES
